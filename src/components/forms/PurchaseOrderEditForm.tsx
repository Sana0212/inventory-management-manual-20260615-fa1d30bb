...
import type { SupplierRecord, ProductRecord, PurchaseOrderRecord, PurchaseOrderItemRecord } from '@/data/types';
...
export function PurchaseOrderEditForm({ id, onSuccess, onCancel }: FormProps) {
  ...
  useEffect(() => {
    // Load PO, Items and master data
    Promise.all([
      fetch(`/api/purchase-orders/${id}`).then((res) => res.json()),
      fetch(`/api/purchase-order-items?purchase_order_id=${id}`).then((res) => res.json()).catch(() => []), 
      fetch('/api/suppliers').then((res) => res.json()),
      fetch('/api/products').then((res) => res.json()),
    ])
      .then(([poData, itemsData, suppliersData, productsData]) => {
        ...
        // Filter items matching current PO
        const filteredItems = Array.isArray(itemsData) 
          ? itemsData.filter((i: any) => i.purchase_order_id === id) 
          : [];

        if (filteredItems.length === 0) {
          setItems([{ product_id: '', quantity_ordered: 1, quantity_received: 0, unit_price: 0 }]);
        } else {
      ...
      .catch((err) => {
        setError(err.message || 'Failed to populate purchase order forms');
        setLoading(false);
      });
  }, [id]);
  ...
  const handleItemValueChange = (index: number, field: 'quantity_ordered' | 'quantity_received' | 'unit_price', value: number) => {
    setItems((prev) =>
      prev.map((item, idx) => {
        if (idx !== index) return item;
  ...
  const handleAddItem = () => {
    setItems((prev) => [...prev, { product_id: '', quantity_ordered: 1, quantity_received: 0, unit_price: 0 }]);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(data.error || 'Failed to save purchase order edits');
      }
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6 bg-white p-6 rounded-lg shadow-sm border border-slate-200">
      <div className="border-b border-slate-100 pb-4">
        ...
        <p className="text-sm text-slate-500">Amend order settings and revise billing lists.</p>
      </div>
        ...
        <div className="space-y-3">
          {items.map((item, index) => (
            <div key={index} className="flex gap-3 items-end bg-slate-50 p-3 rounded-lg border border-slate-200 relative">
              ...
              <div className="w-20">
                <label className="block text-xs font-medium text-slate-500 mb-1">Qty Ord</label>
                <input
                  type="number"
                  min="1"
                  required
              ...
              <div className="w-20">
                <label className="block text-xs font-medium text-slate-500 mb-1">Qty Rec</label>
                <input
                  type="number"
                  min="0"
                  required
                  value={item.quantity_received}
                  onChange={(e) => handleItemValueChange(index, 'quantity_received', Number(e.target.value))}
                  className="w-full rounded border border-slate-300 px-2 py-1.5 text-sm focus:outline-none bg-slate-100"
                />
              </div>
